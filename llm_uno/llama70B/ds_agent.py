...
class LLMUnoAgent:
    ...
    def step(self, state, played_card_log, next_player):
        ...
        if self.is_main_process:
            ...
            for letter in legal_action_letters:
                # Prepending a space helps match tokenization as seen in the debug outputs.
                token_ids = self.tokenizer.encode(" " + letter, add_special_tokens=False)
                if token_ids:
                    legal_token_ids[letter] = token_ids[0]
                else:
                    legal_token_ids[letter] = None
        ...
        # Broadcast final action index to all processes
        action_obj = [action] if self.is_main_process else [0]
        dist.broadcast_object_list(action_obj, src=0)
        final_action = action_obj[0]

        return final_action, legal_letter_probs if self.is_main_process else (None, {})
    ...
    def _generate_prompt(self, state, played_card_log, next_player):
        ...
        template_path = "llm_template_instruct.txt"
        ...
        prompt = template.format(
            help_player=1,  # Player to help (can be dynamic)
